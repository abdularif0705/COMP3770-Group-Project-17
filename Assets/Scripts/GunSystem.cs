...
public class GunSystem : MonoBehaviour
{
    ...
    {
        if (allowButtonHold) 
            shooting = Input.GetKey(KeyCode.Mouse0); // Shoot with left mouse button
        else 
    ...
    private void Shoot()
    {
        ...
        {
            Debug.Log(rayHit.collider.name); // Check if everything works fine

            if (rayHit.collider.CompareTag("Enemy")) // if hits obj with Tag "Enemy" and has the script with the TakeDamage function in it
